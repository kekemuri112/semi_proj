...
<script type="text/javascript">
	...
	/*function getTest(){
		...
		var ths=["회원아이디","<a href='#'>회원이름</a>","회원생일","전화번호","활동포인트","비고"];
		
		for(var i=0;i<6;i++){
			console.log("ths[i]:"+ths[i]);
			var th=document.createElement("th");
			var text=document.createTextNode(ths[i]);
			th.appendChild(text);
			table.appendChild(th);
		}
		approval.appendChild(table);
	}*/
	function getList(pageNum){
		console.log("getList 함수 실행");
		...
		xhr.onreadystatechange = function(){
			if(xhr.readyState==4&&xhr.status==200){
				...
				for(var i=0;i<json.length-1;i++){				
					...
					cafe_numList[i]=users_cafe_num; //리스트에 pk담음
	...
	function denied(users_cafe_num,pageNum){
		var xhr=new XMLHttpRequest();
		xhr.onreadystatechange=function(){
			if(xhr.status==200&&xhr.readyState==4){
				var data=xhr.responseText;
		...
		xhr.open('post',cp+'/cafe/denied.do',true);
		xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
		xhr.send('users_cafe_num='+users_cafe_num);
	}
